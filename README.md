...
- 🌐 **Modern Web Console** — Professional developer interface with the **Ace Code Editor**, syntax highlighting, and instant terminal feedback. Switchable themes (dark / light / lollipop), in-theme dialogs (no jarring browser pop-ups), touch-friendly controls, and a live **running-script** indicator on the list.
...
- 🗂️ **Script Folders (Profiles)** — Organize scripts into folders, shown as **tabs** in the web console and **filter chips** in the app. Create, rename, delete, and move scripts between folders from both UIs. Deleting a folder is non-destructive — its scripts move back to **Uncategorized** rather than being removed. Backups preserve the folder layout.
